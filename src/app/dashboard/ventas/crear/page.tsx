...
function CrearDocumentoVentaContent() {
  ...
  const [activeTab,               setActiveTab]               = useState<"general" | "detalle" | "guia">("general");
  ...
  // ── Guía de Remisión: catálogos y transporte ──────────────────────────────
  const [guiaCatalogs, setGuiaCatalogs]         = useState<any>({});
  const [docTypeOptions, setDocTypeOptions]     = useState<any[]>([]);
  const [filteredSeriesGuia, setFilteredSeriesGuia] = useState<any[]>([]);
  const [loadingSeriesGuia, setLoadingSeriesGuia]   = useState(false);
  ...
  // Si se desactiva la guía mientras esa pestaña está abierta, vuelve a Datos Generales
  useEffect(() => {
    if (!guiaActiva && activeTab === "guia") setActiveTab("general");
  }, [guiaActiva, activeTab]);
  ...
  // El <select> de Moneda no tiene opción vacía "-- Seleccione --": si monedaId queda en ""
  // (p.ej. tras "Limpiar", que resetea el formulario sin re-aplicar el default de catálogos)
  // el navegador simplemente muestra la primera <option> como si estuviera seleccionada,
  // aunque el estado real siga vacío — y la validación al guardar lo rechaza sin motivo
  // visible. Este efecto reaplica el default apenas monedaId quede vacío con catálogos ya
  // cargados, para que lo que se ve coincida con lo que se guarda.
  useEffect(() => {
    if (isEditMode || !catalogs?.monedas?.length || (formData as any).monedaId) return;
    setFormData((prev) => ({
      ...prev,
      monedaId:   catalogs.monedas[0].key?.toString() ?? "",
      tipoCambio: (prev as any).tipoCambio || 1,
    }));
  }, [catalogs, isEditMode, (formData as any).monedaId, setFormData]);
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // Guía de Remisión: catálogos de transporte, almacenes, series y correlativo
  // ─────────────────────────────────────────────────────────────────────────────

  const extractArrayGuia = <T,>(data: unknown): T[] => {
  ...
  // ── Catálogos de transporte (transportista / unidad / conductor) ─────────────
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setUnidades(
          resU.status === "fulfilled" && resU.value?.isSuccess
            ? extractArrayGuia<UnidadTransporte>(resU.value.data) : []
        );
        setConductores(
          resC.status === "fulfilled" && resC.value?.isSuccess
            ? extractArrayGuia<Conductor>(resC.value.data) : []
        );
        setTransportistas(
          resT.status === "fulfilled" && resT.value?.isSuccess
            ? extractArrayGuia<Transportista>(resT.value.data) : []
        );
      } catch {
  ...
  // ── Catálogos base de guía (bootstrap): usa el almacén que ya esté en el
  //    store, o el primero disponible una vez que cargue AlmacenInicioJson ───
  useEffect(() => {
    const bootstrapId = guiaForm.almacenId || EMPRESA_ID;
    guiaRemisionService
      .getFormDropdowns(EMPRESA_ID, bootstrapId)
      .then((res) => {
        if (!res.isSuccess) return;
        const data = res.data;
        setGuiaCatalogs(data);
        ...
        if (guiaForm.almacenId) {
          prevAlmacenRef.current = guiaForm.almacenId;
          setFilteredSeriesGuia(
            (data.serieGuiaJson || []).filter((s: any) => s.groupkey === "X029")
          );
        }
      })
  ...
  // ── Recarga catálogos de guía cuando cambia el almacén de origen ─────────────
  useEffect(() => {
    if (!guiaForm.almacenId || guiaForm.almacenId === prevAlmacenRef.current) return;
    prevAlmacenRef.current = guiaForm.almacenId;
    setLoadingSeriesGuia(true);
    guiaRemisionService
      .getFormDropdowns(EMPRESA_ID, guiaForm.almacenId)
      .then((res) => {
        if (res.isSuccess) {
          const data = res.data;
          setGuiaCatalogs(data);
      ...
      .finally(() => setLoadingSeriesGuia(false));
  }, [guiaForm.almacenId]);

  // ── Auto-seleccionar primera serie de guía al cambiar almacén ────────────────
  useEffect(() => {
    if (filteredSeriesGuia.length === 0) return;
    if (!guiaForm.serie || !filteredSeriesGuia.find((s: any) => s.key === guiaForm.serie)) {
      updateGuia("serie", filteredSeriesGuia[0].key);
    }
  }, [filteredSeriesGuia, guiaForm.serie, updateGuia]);

  // ── Correlativo automático de guía ────────────────────────────────────────────
  useEffect(() => {
    if (isManualCorrelativo || !guiaForm.serie || !guiaCatalogs.correlativoJson) return;
    const searchKey = `X029 ${guiaForm.serie}`;
    const obj = guiaCatalogs.correlativoJson.find((c: any) => c.key?.trim() === searchKey.trim());
    if (obj) {
      const lastStr = String(obj.value || "");
      const lastNum = parseInt(lastStr, 10);
      if (!isNaN(lastNum)) {
        const next = (lastNum + 1).toString().padStart(lastStr.length, "0");
        if (guiaForm.correlativo !== next) updateGuia("correlativo", next);
      } else if (lastStr && guiaForm.correlativo !== lastStr) {
        updateGuia("correlativo", lastStr);
      }
    } else if (guiaForm.correlativo !== "0000001") {
      updateGuia("correlativo", "0000001");
    }
  }, [guiaForm.serie, guiaCatalogs.correlativoJson, isManualCorrelativo, guiaForm.correlativo, updateGuia]);

  // ── Reglas por motivo de traslado ─────────────────────────────────────────────
  const getMotivoRulesGuia = (motivoId: string) => {
    switch (motivoId) {
  ...
  const currentRulesGuia = useMemo(
    () => getMotivoRulesGuia(guiaForm.motivoTrasladoId || ""),
    [guiaForm.motivoTrasladoId]
  );

  const getAddressGuia = useCallback((sourceType: string) => {
    if (sourceType === "ALM_ORIG")
      return guiaCatalogs.AlmacenInicioJson?.find((x: any) => x.key === guiaForm.almacenId)?.aux || "";
    if (sourceType === "ALM_DEST")
      return guiaCatalogs.AlmacenDestinoJson?.find((x: any) => x.key === guiaForm.almacenDestinoId)?.aux || "";
    if (sourceType === "CLI")
      return guiaCatalogs.clienteJson?.find((x: any) => x.key === (formData as any).clienteId)?.aux || "";
    return "";
  }, [guiaCatalogs, guiaForm.almacenId, guiaForm.almacenDestinoId, (formData as any).clienteId]);

  useEffect(() => {
    const partida = getAddressGuia(currentRulesGuia.start);
    const llegada = currentRulesGuia.end === "ALM_DEST"
      ? getAddressGuia("ALM_DEST")
      : getAddressGuia(currentRulesGuia.end);
    if (partida) updateGuia("puntoPartida", partida);
    if (llegada) updateGuia("puntoLlegada", llegada);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [guiaForm.almacenId, guiaForm.almacenDestinoId, (formData as any).clienteId, guiaCatalogs, currentRulesGuia.start, currentRulesGuia.end]);

  // Auto-completa puntoLlegada con la dirección del pedido de venta importado,
  // o la dirección principal del cliente si no hubiera ninguna guardada.
  useEffect(() => {
    if (!(formData as any).clienteId || currentRulesGuia.end !== "CLI") return;
    if ((formData as any).clienteDireccion) {
      updateGuia("puntoLlegada", (formData as any).clienteDireccion);
      return;
    }
    clienteService.getById((formData as any).clienteId)
      .then((cli: any) => {
        if (cli?.direccion) updateGuia("puntoLlegada", cli.direccion);
      })
      .catch(() => { /* silent */ });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [(formData as any).clienteId, (formData as any).clienteDireccion, currentRulesGuia.end, updateGuia]);

  // El DateInput de fecha de traslado muestra `today` como fallback visual cuando
  // guiaForm.fechaTraslado está vacío, pero eso no escribe el valor real en el store
  // (queda "" hasta que el usuario toca el picker). Esto autocompleta el store para
  // que lo que se ve coincida con lo que se valida/envía al guardar.
  useEffect(() => {
    if (guiaActiva && !guiaForm.fechaTraslado) updateGuia("fechaTraslado", today);
  }, [guiaActiva, guiaForm.fechaTraslado, today, updateGuia]);

  const handleFechaTrasladoChangeGuia = (value: string) => {
    if (value < today)
      toast.warning("La fecha de traslado no puede ser menor a la fecha actual");
    updateGuia("fechaTraslado", value);
  };

  const handleMotivoChangeGuia = (value: string) => {
    updateGuia("motivoTrasladoId", value);
    if (value !== "13") updateGuia("otroMotivoTraslado", "");
    if (value !== "04") updateGuia("almacenDestinoId", "");
  };
  ...
  return (
    <div className="p-6 pb-20">
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className={`col-span-12 ${activeTab === "detalle" ? "" : "lg:col-span-8"}`}>
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="flex border-b border-slate-200 bg-slate-50">
              ...
              <button
                type="button"
                onClick={() => guiaActiva && setActiveTab("guia")}
                title={!guiaActiva ? "Activa la Guía de Remisión desde Condiciones de Pago" : undefined}
                className={`flex items-center gap-2 px-6 py-3.5 text-xs font-bold uppercase tracking-wide border-b-2 transition-colors ${
                  activeTab === "guia"
                    ? "border-blue-600 text-blue-600 bg-white"
                    : !guiaActiva
              ...
              >
                <IconTruck size={15} /> Guía Remisión
                {!guiaActiva && <IconLock size={11} className="ml-0.5 text-slate-300" />}
              </button>
            </div>
                ...
                <div className="border-t border-slate-100 pt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <input id="chk-guia" type="checkbox" checked={guiaActiva} className="w-4 h-4 accent-indigo-600 cursor-pointer"
                      onChange={(e) => { const c = e.target.checked; setGuiaActiva(c); if (c) setActiveTab("guia"); }} />
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="p-6 space-y-6">
                ...
                {!(formData as any).clienteId && (
                  <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 text-amber-800">
                    ...
                    <p className="text-sm">
                      No hay cliente seleccionado en <button type="button" onClick={() => setActiveTab("general")} className="font-bold underline underline-offset-2 hover:text-amber-900">Datos Generales</button>. Selecciónalo antes de guardar la guía.
                    </p>
                  </div>
                )}

                {(detalles as any[]).length > 0 && (
                  <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-100 rounded-xl px-5 py-3 text-indigo-700 text-sm">
                    <IconInfoCircle size={16} className="shrink-0" />
                    <span>
                      <strong>{(detalles as any[]).length} ítem{(detalles as any[]).length !== 1 ? "s" : ""}</strong> del{" "}
                      <button type="button" onClick={() => setActiveTab("detalle")} className="font-bold underline underline-offset-2 hover:text-indigo-900">Detalle</button>{" "}
                      se incluirán en esta guía.
                    </span>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2 flex flex-col gap-1.5">
                      ...
                      <select
                        value={guiaForm.motivoTrasladoId}
                        onChange={(e) => handleMotivoChangeGuia(e.target.value)}
                      ...
                      >
                        <option value="">Seleccione...</option>
                        {(Array.isArray(guiaCatalogs.motivoTrasladoJson)
                          ? guiaCatalogs.motivoTrasladoJson
                          : Array.isArray(guiaCatalogs.motivoTasladoJson)
                          ? guiaCatalogs.motivoTasladoJson
                          : [])
                    ...
                    {currentRulesGuia.allowManual && (
                      <div className="md:col-span-2">
                        <FormInputGuia
                          label="Especifique el Motivo"
                          value={guiaForm.otroMotivoTraslado}
                          onChange={(e: any) => updateGuia("otroMotivoTraslado", e.target.value)}
                        />
                      </div>
                    )}
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <select
                          value={guiaForm.almacenId}
                          onChange={(e) => {
                            updateGuia("almacenId", e.target.value);
                            const addr = guiaCatalogs.AlmacenInicioJson?.find((x: any) => x.key === e.target.value)?.aux || "";
                            if (addr) updateGuia("puntoPartida", addr);
                          }}
                          className="w-full border border-slate-200 p-2.5 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                        >
                          <option value="">Seleccione...</option>
                          {(guiaCatalogs.AlmacenInicioJson || []).map((x: any) => (
                      ...
                      <FormInputGuia
                        ...
                        value={guiaForm.puntoPartida}
                        onChange={(e: any) => updateGuia("puntoPartida", e.target.value)}
                      />
                    </div>

                    <div className="space-y-3">
                      {currentRulesGuia.showDestAlmacen ? (
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Almacén Destino</label>
                          <select
                            value={guiaForm.almacenDestinoId}
                            onChange={(e) => {
                              updateGuia("almacenDestinoId", e.target.value);
                              const addr = guiaCatalogs.AlmacenDestinoJson?.find(
                                (x: any) => x.key === e.target.value
                              )?.aux || "";
                              updateGuia("puntoLlegada", addr);
                            }}
                            className="w-full border border-slate-200 p-2.5 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                          >
                            <option value="">Seleccione...</option>
                            {(guiaCatalogs.AlmacenDestinoJson || [])
                              .filter((x: any) => x.key !== guiaForm.almacenId)
                              .map((x: any) => (
                                <option key={x.key} value={x.key}>{x.value}</option>
                              ))}
                          </select>
                      ...
                      <FormInputGuia
                        ...
                        value={guiaForm.puntoLlegada}
                        onChange={(e: any) => updateGuia("puntoLlegada", e.target.value)}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-200 pt-6">
                  <SectionTitle title="Información General de la Guía" icon={IconFileDescription} />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Tipo Documento</label>
                      <select value="X029" disabled
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <select
                          ...
                          onChange={(e) => updateGuia("serie", e.target.value)}
                          disabled={filteredSeriesGuia.length === 0 || loadingSeriesGuia}
                        ...
                        >
                          {(filteredSeriesGuia.length === 0 || loadingSeriesGuia) && (
                            <option value="">{loadingSeriesGuia ? "Cargando..." : "-"}</option>
                          )}
                          {filteredSeriesGuia.map((s: any) => (
                      ...
                      <div className="flex flex-col gap-1.5">
                          ...
                          <div className="flex items-center gap-1 cursor-pointer">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (!e.target.checked) updateGuia("correlativo", "");
                              }}
                        ...
                        <input
                          value={guiaForm.correlativo || ""}
                          onChange={(e) => isManualCorrelativo && updateGuia("correlativo", e.target.value)}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <DateInput
                        ...
                        name="fechaTraslado"
                        value={guiaForm.fechaTraslado || today}
                        onChange={(e) => handleFechaTrasladoChangeGuia(e.target.value)}
                      />
                    </div>
                  </div>
                </div>
                ...
                {/* ── Transporte ── */}
                <div className="border-t border-slate-200 pt-6">
                  <SectionTitle title="Transporte" icon={IconPackage} />
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) => updateGuia("transportista", e.target.value)}
                        className="w-full border border-slate-200 p-2.5 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                      >
                        <option value="">Seleccione...</option>
                      ...
                      {guiaForm.transportista && (
                        <p className="text-[11px] text-slate-500 italic ml-1">
                          Dirección:{" "}
                          {transportistas.find((t) => t.transportistaId === guiaForm.transportista)?.direccion || "No registrada"}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        value={guiaForm.unidadTransporte || ""}
                        onChange={(e) => updateGuia("unidadTransporte", e.target.value)}
                        className="w-full border border-slate-200 p-2.5 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                      >
                        <option value="">Seleccione...</option>
                        {unidades.map((u) => (
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        value={guiaForm.conductor || ""}
                        onChange={(e) => updateGuia("conductor", e.target.value)}
                        className="w-full border border-slate-200 p-2.5 rounded-lg text-xs outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
                      >
                        <option value="">Seleccione...</option>
                        {conductores.map((c) => (
                ...
                <div className="border-t border-slate-200 pt-6">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea
                      value={guiaForm.observacion || ""}
                      onChange={(e) => updateGuia("observacion", e.target.value)}
        ...
        {(activeTab === "general" || activeTab === "guia") && (
