        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InfoField
                ...
                value={
                  cot.cliente?.descripcion
                    ? `${cot.cliente.descripcion}${cot.cliente.num_docident ? ` (${cot.cliente.num_docident})` : ""}`
                    : cot.clienteId
                }
                fullWidth
              />
              ...
              <InfoField
                ...
                value={cot.tipoentrega?.descripcion || cot.tipoentregaId}
              />
          ...
          <div className="space-y-3">
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
              <InfoField label="Forma de Pago"    value={cot.formapago?.descripcion   || cot.formaspagoId} />
              <InfoField label="Condición de Pago" value={cot.condicionpago?.descripcion ? `${cot.condicionpago.descripcion}${cot.condicionpago.dias ? ` (${cot.condicionpago.dias} días)` : ""}` : cot.condicionPago} />
              <InfoField
                label="Moneda"
                value={cot.moneda?.descripcion ? `${cot.moneda.descripcion}${cot.moneda.abreviatura ? ` (${cot.moneda.abreviatura})` : ""}` : cot.monedaId}
              />
              <InfoField label="Tipo de Cambio" value={cot.tipoCambio ? fmt(cot.tipoCambio) : undefined} />
              {cot.saldo != null && cot.saldo !== cot.total && (
                <InfoField label="Saldo" value={fmt(cot.saldo)} />
              )}
            </dl>
          </div>
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-200">
              <table className="w-full text-sm">
                <thead className="bg-slate-100 text-slate-600">
                  <tr>
                    {["#", "Producto", "Presentación", "Cantidad", "Precio", "Desc. %", "Importe", "IGV"].map((h) => (
